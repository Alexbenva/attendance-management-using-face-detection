...
def get_button(window, text, color, command, fg='white'):
    return tk.Button(window, text=text, bg=color, fg=fg, command=command, font=('Arial', 14, 'bold'), relief=tk.FLAT, width=20, height=2)
...
def load_known_faces():
    ...
    global known_face_encodings_students, known_face_ids_students, known_face_encodings_staff, known_face_ids_staff
    ...
    try:
        with conn.cursor() as cursor:
            ...
            known_face_encodings_students = [np.array(json.loads(s['face_encoding'])) for s in students]
            ...
            known_face_encodings_staff = [np.array(json.loads(s['face_encoding'])) for s in staff]
...
def recognize(frame, user_type):
    ...
    encodings_to_check, ids_to_check = (known_face_encodings_students, known_face_ids_students) if user_type == 'student' else (known_face_encodings_staff, known_face_ids_staff)
...
def add_student(name, reg_no, department, encoding):
    ...
    try:
        with conn.cursor() as cursor:
            sql = "INSERT INTO students (name, reg_no, department, face_encoding) VALUES (%s, %s, %s, %s)"
...
def get_all_students():
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("SELECT reg_no, name, department FROM students ORDER BY name")
...
def mark_student_entry(reg_no):
    ...
    try:
        with conn.cursor() as cursor:
            ...
            sql = "INSERT INTO attendance_students (reg_no, date, time_in) VALUES (%s, %s, %s)"
            cursor.execute(sql, (reg_no, today, now_time))
        conn.commit()
        return True, "Entry marked."
    except db.pymysql.err.IntegrityError:
        return False, "Already marked entry today."
    finally:
        if conn: conn.close()
...
def mark_student_exit(reg_no):
    ...
    try:
        with conn.cursor() as cursor:
            today = date.today().strftime('%Y-%m-%d')
            now_time = datetime.now().strftime('%H:%M:%S')
            sql = "UPDATE attendance_students SET time_out = %s WHERE reg_no = %s AND date = %s AND time_out IS NULL"
            rows_affected = cursor.execute(sql, (now_time, reg_no, today))
            if rows_affected > 0:
                conn.commit()
...
def get_class_schedule():
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("SELECT hour_name, start_time, end_time, entry_deadline, early_entry_minutes FROM class_schedule ORDER BY start_time")
            ...
            for row in schedule_data:
                for key, value in row.items():
                    if isinstance(value, timedelta):
                        ...
                        row[key] = datetime.strptime(f"{hours:02}:{minutes:02}:{seconds:02}", '%H:%M:%S').time()
...
def add_staff(name, staff_id, course_id, subject, encoding):
    ...
    try:
        with conn.cursor() as cursor:
            sql = "INSERT INTO staff (name, staff_id, course_id, subject, face_encoding) VALUES (%s, %s, %s, %s, %s)"
...
def mark_staff_entry(staff_id, hour):
    ...
    try:
        with conn.cursor() as cursor:
            today = date.today().strftime('%Y-%m-%d')
            now_time = datetime.now().strftime('%H:%M:%S')
            sql = "INSERT INTO attendance_staff (staff_id, date, hour, time_in, status) VALUES (%s, %s, %s, %s, 'Present')"
...
def mark_staff_exit(staff_id):
    ...
    try:
        with conn.cursor() as cursor:
            ...
            now_time = datetime.now().strftime('%H:%M:%S')
            sql = """
                UPDATE attendance_staff SET time_out = %s 
