...
def create_connection(db: str = None):
    ...
    try:
        ...
        if "Unknown database" in str(e): return None
...
def init_database():
    ...
    try:
        server_conn = pymysql.connect(host=DB_HOST, user=DB_USER, password=DB_PASSWORD, cursorclass=DictCursor)
        ...
        with conn.cursor() as cur:
            cur.execute("""
                CREATE TABLE IF NOT EXISTS students (
                    id INT AUTO_INCREMENT PRIMARY KEY, reg_no VARCHAR(50) UNIQUE NOT NULL,
                    name VARCHAR(200) NOT NULL, department VARCHAR(100), face_encoding LONGTEXT NOT NULL,
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
            """)
            cur.execute("""
                CREATE TABLE IF NOT EXISTS staff (
                    id INT AUTO_INCREMENT PRIMARY KEY, staff_id VARCHAR(50) UNIQUE NOT NULL,
                    name VARCHAR(200) NOT NULL, course_id VARCHAR(100), subject VARCHAR(100),
                    face_encoding LONGTEXT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
            """)
            ...
            cur.execute("""
                CREATE TABLE IF NOT EXISTS attendance_students (
                    id INT AUTO_INCREMENT PRIMARY KEY, reg_no VARCHAR(50) NOT NULL, date DATE NOT NULL,
                    time_in TIME NOT NULL, time_out TIME, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    FOREIGN KEY (reg_no) REFERENCES students(reg_no) ON DELETE CASCADE,
                    UNIQUE KEY unique_attendance_student (reg_no, date));
            """)
            cur.execute("""
                CREATE TABLE IF NOT EXISTS attendance_staff (
                    id INT AUTO_INCREMENT PRIMARY KEY, staff_id VARCHAR(50) NOT NULL, date DATE NOT NULL,
                    hour VARCHAR(20) NOT NULL, time_in TIME NOT NULL, time_out TIME, status VARCHAR(10) DEFAULT 'Present',
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (staff_id) REFERENCES staff(staff_id) ON DELETE CASCADE,
            ...
            if cur.fetchone()['count'] == 0:
                ...
                cur.executemany(
                    "INSERT INTO class_schedule (hour_name, start_time, end_time, entry_deadline, early_entry_minutes) VALUES (%s, %s, %s, %s, %s)",
                    schedule_data
                )
